...
# IBM Cloud Object Storage Credentials from environment variables
# cos_credentials = {
#     "access_key_id": os.getenv("IBM_ACCESS_KEY_ID"),
#     "secret_access_key": os.getenv("IBM_SECRET_ACCESS_KEY"),
...
# Function to read CSV from COS
# print(cos_credentials)


def read_csv_from_cos(file_key):
    obj = cos_client.get_object(Bucket=cos_credentials["bucket"], Key=file_key)
    return pd.read_csv(BytesIO(obj['Body'].read()))

# Function to read Parquet from COS into Spark


def read_parquet_from_cos(file_key, spark):
    obj = cos_client.get_object(Bucket=cos_credentials["bucket"], Key=file_key)
...
# Load data from COS
processed_data_key = "processed_customer_purchase_behavior.csv"
predictions_data_key = "Random Forest.parquet/part-00000-01afd644-f829-4889-8629-0c01c401bf97-c000.snappy.parquet"

data_pd = read_csv_from_cos(processed_data_key)
predictions_df = read_parquet_from_cos(predictions_data_key, spark)
